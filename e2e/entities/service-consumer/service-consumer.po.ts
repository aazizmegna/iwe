...
export class ServiceConsumerComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Service Consumers found.'));
  entities = element.all(by.css('page-service-consumer ion-item'));
...
export class ServiceConsumerUpdatePage {
...
export class ServiceConsumerDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
