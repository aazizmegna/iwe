...
export class TextPostComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Text Posts found.'));
  entities = element.all(by.css('page-text-post ion-item'));
...
export class TextPostUpdatePage {
...
export class TextPostDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
